...
class ModelTrainer:
    ...
    def validate_model(self, model: EnsembleYieldPredictor,
        ...
        if metrics['r2_score'] < thresholds['min_r2_score']:
            ...
            failed_criteria.append(f"R² score ({metrics['r2_score']:.3f}) below threshold ({thresholds['min_r2_score']})")
        ...
        if metrics['accuracy_within_20_percent'] < thresholds['min_accuracy_20_percent']:
            validation_passed = False
            failed_criteria.append(f"20% accuracy ({metrics['accuracy_within_20_percent']:.1f}%) below threshold ({thresholds['min_accuracy_20_percent']}%)")

        if metrics['avg_confidence'] < thresholds['min_avg_confidence']:
            validation_passed = False
            failed_criteria.append(f"Average confidence ({metrics['avg_confidence']:.1f}%) below threshold ({thresholds['min_avg_confidence']}%)")
